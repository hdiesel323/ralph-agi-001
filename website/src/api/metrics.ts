 ...
 */
export async function resetMetrics(): Promise<{ status: string; message: string }> {
  const response = await apiClient.post<{ status: string; message: string }>('/api/metrics/reset');
  return response.data;
}
 ...
 */
export async function addTokens(inputTokens: number, outputTokens: number): Promise<Metrics> {
