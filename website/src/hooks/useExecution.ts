...
export function useExecution(options: UseExecutionOptions = {}): UseExecutionReturn {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch execution status'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const start = useCallback(async (maxConcurrent = 3, maxTasks?: number) => {
    try {
      setError(null);
      await startExecution({ max_concurrent: maxConcurrent, max_tasks: maxTasks });
      await refresh();
    } catch (err) {
      setError(err instanceof Error ? err : new Error('Failed to start execution'));
      throw err;
    }
  }, [refresh]);

  const stop = useCallback(async (wait = true) => {
    try {
      setError(null);
      await stopExecution(wait);
      await refresh();
    } catch (err) {
      setError(err instanceof Error ? err : new Error('Failed to stop execution'));
      throw err;
    }
  }, [refresh]);
  ...
  const cleanup = useCallback(async (force = false): Promise<number> => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to cleanup worktrees'));
      throw err;
    }
  }, []);
